...
class Phase23PerformanceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_phase23_performance_targets_are_met(int $donorCount): void
    {
        ...
        fwrite(STDOUT, sprintf(
            "\n[Phase23][%d donors] query=%.2fms | matching=%.2fms | api=%.2fms | notifications=%.2fms\n",
            $donorCount,
            $queryMs,
            $matchingMs,
            $apiMs,
    ...
    private function seedDonorDataset(int $count): void
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            $userRows[] = [
                'name' => 'Perf Donor '.$i,
                'email' => 'phase23.donor.'.$count.'.'.$i.'@example.com',
                'password' => 'password123',
                'role' => 'donor',
                'created_at' => $now,
                'updated_at' => $now,
        ...
        $users = DB::table('users')
            ...
            ->where('email', 'like', 'phase23.donor.'.$count.'.%@example.com')
            ->orderBy('id')
            ->get();
